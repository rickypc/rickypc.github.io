...
import { barsUpdate, barUpdate, create, increment, MultiBar, setTotal, stop } from 'cli-progress';
...
const globMock = glob as Mocked<typeof glob>;
...
// Bun's mock.module is not hoisted: read the live bindings only after registration.
const accessMock = access as Mocked<typeof access>;
const execSyncMock = execSync as Mocked<typeof execSync>;
const spawnMock = spawn as Mocked<typeof spawn>;
...
function makeCli(actions: CliActions) {
  return {
    action: mock(function action(this: unknown, fn) {
      actions.action = fn;
      return this;
    }),
    command: mock(function command(this: unknown, name) {
      actions.command = name;
      return this;
    }),
    description: mock(function description(this: unknown) {
      return this;
    }),
    option: mock(function option(this: unknown) {
      return this;
    }),
  };
}
...
const phrases = [
  { content: 'One', path: '#lib/path/one.md' },
  { content: 'Two', path: '#lib/path/_ricky_huang.md' },
  { content: 'Three', path: '#lib/path/three.md' },
];

for (const phrase of phrases) {
  mock.module(phrase.path, () => ({
    default: {
      transliteration: {
...
describe(`plugins.${name}.fileResolve`, () => {
  const siteDir = '/root';
  test.each([
...
describe(`plugins.${name}.generateAudio: progress bar on update`, () => {
  const { length } = audio;
  ...
  test('writes audio files and updates progress bar once per m4a', async () => {
    ...
    global.fetch = mock().mockImplementation(successFetch) as unknown as typeof fetch;
    const pipers: Piper[] = [];
    let port = 5001;
    ...
    expect((global.fetch as Mocked<typeof global.fetch>).mock.calls).toEqual(piperRequests);
...
describe(`plugins.${name}.generateAudio: progress bar on skipped`, () => {
  const { length } = audio;
  const out = `${name}-test-out`;
  const siteDir = tmpdir();
  // After siteDir assignment.
...
describe(`plugins.${name}.generateAudio: piper failed`, () => {
  const { length } = audio;
  const out = `${name}-test-out`;
  const siteDir = tmpdir();
  // After siteDir assignment.
  ...
  test('logs error and returns early when Piper is missing', async () => {
    ...
    expect(consoleMock).toHaveBeenCalledWith(expect.stringContaining('Piper not found'));
...
describe(`plugins.${name}.generateAudio: exits error`, () => {
  const { length } = audio;
  const out = `${name}-test-out`;
  const siteDir = tmpdir();
  // After siteDir assignment.
...
describe(`plugins.${name}.generateAudioMetadata`, () => {
...
describe(`plugins.${name}.generateAudioTrack`, () => {
  ...
  const tracks = new Map([
    [path, { album: 'album', description: 'description', genre: 'genre', track: 1 }],
  ]);
  ...
  test('should handle a missing track gracefully using the fallback object', async () => {
    global.fetch = mock().mockImplementation(successFetch) as unknown as typeof fetch;
    spawnMock.mockReturnValue(ffmpeg());
    await Plugin.generateAudioTrack({
      generator,
...
describe(`plugins.${name}.generatePdf`, () => {
  const { length } = pdf;
  const out = `${name}-test-out`;
  const siteDir = tmpdir();
  // After siteDir assignment.
...
describe(`plugins.${name}.inlineAboveFold`, () => {
...
describe(`plugins.${name}.outputPaths`, () => {
...
describe(`plugins.${name}.piperServer`, () => {
  test('spawns piper server and resolves when ready message appears', async () => {
    ...
    expect(spawnMock).toHaveBeenCalledWith(`${process.env.HOME}/.venv/default/bin/python`, [
      '-m',
      'piper.http_server',
...
describe(`plugins.${name}.postBuild`, () => {
...
describe(`plugins.${name}.stale`, () => {
  ...
  ])('%s -> expected=%s', async (_, expected, accessReturn, stats) => {
    accessMock[`mock${accessReturn instanceof Error ? 'Rejected' : 'Resolved'}ValueOnce`](
      accessReturn as never,
    );
...
describe(`plugins.${name}.plugin API (default export)`, () => {
  ...
  test('extendCli action: explicit args resolve config path and invoke postBuild side-effects', async () => {
    ...
    const cli = {
      action: mock().mockImplementation(function action(fn) {
        actions.action = fn;
        return cli;
      }),
      command: mock().mockImplementation(function command(cmd) {
        actions.command = cmd;
        return cli;
      }),
      description: mock().mockReturnThis(),
      option: mock().mockReturnThis(),
    };
    (loadFreshModule as Mocked<typeof loadFreshModule>).mockResolvedValue(siteConfig);
...
describe(`plugins.${name}.plugin API (default export): error handling`, () => {
