...
describe('home.Hats', () => {
  test('renders all hats with proper structure and content', () => {
    ...
    // eslint-disable-next-line testing-library/no-node-access
    expect((root as Element).childElementCount).toBe(4);

    // eslint-disable-next-line testing-library/no-node-access
    const articles = (root as Element).querySelectorAll('article');
    ...
    // eslint-disable-next-line testing-library/no-node-access
    fireEvent.click(articles[0].querySelector('.details') as Element);
  });
