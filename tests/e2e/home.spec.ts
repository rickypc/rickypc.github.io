...
test.describe
  .serial('shared page tests', () => {
      ...
      await band(4, async (index) => {
        ...
        await Promise.all(
          (
            await page
              .locator(
                `main article:nth-of-type(${nth}) h2 span[class*='phrases_'] span[class*='word_']`,
              )
              .all()
          ).map(async (word) => expect(word).toBeVisible()),
        );
        expect(await page.textContent(`main article:nth-of-type(${nth}) p`)).toMatchSnapshot(
          `paragraph-${nth}.txt`,
        );
      });
    });
    ...
    test('has self image on mobile', async ({}, testInfo) => {
      if (!mobile(testInfo)) {
        return;
      }
    ...
    // biome-ignore lint/correctness/noEmptyPattern: -
    test('has self image on desktop', async ({}, testInfo) => {
      if (mobile(testInfo)) {
        return;
      }
    ...
    test('has social links', async () => {
      await band(2, async (index) => {
        await expect(
          page.locator(`main ul[class*='social_'] li:nth-of-type(${index + 1}) a`),
        ).toBeVisible();
      });
    });
  });
...
test.describe('isolated tests', () => {
  ...
  test('has greeting', async ({ page }) => {
    ...
    await hasPlayback({
      page,
      selector: 'main header [class*="greeting_"] [class*="controls_"] [class*="control_"]',
      url,
    });
    const nth = 1;
    // After nth assignment.
    const locator = page.locator(
      `main header [class*="greeting_"]>[class*="pronunciation_"]>span:nth-of-type(${nth})`,
    );
