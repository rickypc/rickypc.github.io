/*!
 * Copyright © 2015 Richard Huang <rickypc@users.noreply.github.com>
 * All rights reserved.
 */

export default {
  chinese: {
    children: '嗡 達啦 達啦 地利 地利 杜露 杜露 易笛 威易笛 · 砸列 砸列 · 砸薩列 砸薩列 · 固苏美 固苏玛哇垒 · 易利 密利 积地 · 止致 炸啦玛巴纳雅 · 跛羅麼 秫馱薩怛嚩 摩訶 迦盧尼迦 梭哈',
    title: '十一面观音陀罗尼',
  },
  path: import.meta.url,
  sanskrit: {
    children: 'ॐ धर धर धीरे धीरे धुरु धुरु ईट्टे विट्टे । चले चले प्रचले प्रचले कुसुमे कुसुमवरे । इली मिल् विटि । चित्ति ज्वालम् आपनाय । परम शुद्धसत्त्व महा कारुणिक स्वाहा',
    title: 'एकादशमुखलोकेश्वर',
  },
  tibetan: {
    children: 'ༀ་དྷ་ར་དྷ་ར་དྷཱི་རེ་དྷཱི་རེ་དྷུ་རུ་དྷུ་རུ་ཨཱི་ཊྚེ་བི་ཊྚེ། ཙ་ལེ་ཙ་ལེ་པྲ་ཙ་ལེ་པྲ་ཙ་ལེ་ཀུ་སུ་མེ་ཀུ་སུ་མ་བ་རེ། ཨི་ལཱི་མི་ལ་བི་ཊི། ཙི་ཏྟི་ཛྭཱ་ལ་མ་ཨཱ་པ་ནཱ་ཡ། པ་ར་མ་ཤུ་དྡྷ་ས་ཏྟྭ་མ་ཧཱ་ཀཱ་རུ་ཎི་ཀ་སྭཱ་ཧཱ',
    repeat: { condensed: 71, roll: 25, wheel: 23 },
    title: 'བཅུ་གཅིག་ཞལ',
    typography: {
      thangka: { default: 10, double: 18, single: 42.5 },
    },
  },
  translation: {
    title: 'Eleven-Faced Avalokiteśvara',
  },
  transliteration: {
    children: 'oṃ dhara dhara dhīre dhīre dhuru dhuru iṭṭe viṭṭe । cale cale । pracale pracale । kusume kusumavare । ili mili viṭi । citti jvālaṃ āpanāya । parama śuddhasattva mahā kāruṇika svāhā',
    repetition: 3,
    // The eleven-faced lord of the world (Avalokiteśvara).
    title: 'Ekādaśamukhalokeśvara',
  },
};
