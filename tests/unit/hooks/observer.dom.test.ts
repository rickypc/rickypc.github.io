...
const useIsBrowserMock = useIsBrowser as Mocked<typeof useIsBrowser>;
const useLocationMock = useLocation as Mocked<typeof useLocation>;

const spies = {
  doc: {
    add: spyOn(document, 'addEventListener'),
    remove: spyOn(document, 'removeEventListener'),
  },
  win: {
    add: spyOn(window, 'addEventListener'),
    remove: spyOn(window, 'removeEventListener'),
  },
};
...
describe('useMedia', () => {
  ...
  beforeEach(() => {
    ...
    window.matchMedia = mock((query): MediaQueryList => {
      const mql = {
        addEventListener: mock((_, cb) => {
          mediaQueryListeners.set(query, cb);
        }),
        matches: query.includes('min-width'),
        media: query,
        removeEventListener: mock((_, cb) => {
...
describe('useReadingTime', () => {
  ...
  test('returns 0 when not in browser', () => {
  ...
  test('returns 0 when element is not found', () => {
  ...
  test('calculates reading time based on text content', async () => {
    document.body.innerHTML = `
  ...
  test('updates when selector changes', async () => {
    document.body.innerHTML = `
...
describe('useVisibility (Browser)', () => {
  ...
  test('does nothing if ref.current is falsy, but reacts to visibilitychange', () => {
    spies.win.add.mockImplementation((event: string, cb: EventListenerOrEventListenerObject) => {
      if (event === 'focus') {
