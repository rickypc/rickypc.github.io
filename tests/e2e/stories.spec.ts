...
test.describe
  .serial('shared page tests', () => {
    ...
    test('has active navigation', async ({}, testInfo) =>
      hasActiveNavigation({ name: 'Stories', page, testInfo }));

    test('has 4 stories', async () => {
      await band(4, async (index) => {
        const nth = index + 1;
        expect(
          await page.textContent(`main section article:nth-of-type(${nth}) h2`),
        ).toMatchSnapshot(`story-header-${nth}.txt`);
        expect(
          await page.textContent(`main section article:nth-of-type(${nth}) p`),
        ).toMatchSnapshot(`story-content-${nth}.txt`);
        expect(
          await page.textContent(`main section article:nth-of-type(${nth}) address`),
