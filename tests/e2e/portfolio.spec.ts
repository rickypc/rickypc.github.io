...
test.describe.serial('shared page tests', () => {
  ...
  test('has active navigation', async ({}, testInfo) => hasActiveNavigation('Portfolio', page, testInfo));

  test('has 12 portfolios', async () => {
...
test.describe('isolated tests', () => {
  ...
  ['Dark', 'Light'].forEach((theme) => {
    test(
      ...
      async ({ page }, testInfo) => hasScreenshot(page, testInfo, theme, url, 'article[class^="portfolio_"]:nth-of-type(10) div[class^="indicators_"]'),
    );
  });
});
