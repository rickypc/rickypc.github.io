...
test.describe
  .serial('shared page tests', () => {
    ...
    test('has GEO JSON-LD blocks', async () => hasJsonLd({ faqId: '/about#faq', page }));
    test('has correct header', async () => hasHeader({ page }));
    // biome-ignore lint/correctness/noEmptyPattern: -
    test('has active navigation', async ({}, testInfo) =>
      hasActiveNavigation({ name: 'About Me', page, testInfo }));

    test('has types', async () => {
      await Promise.all(
        ['Transformer People Type', 'Transactor Task Type'].map(async (label, index) => {
          // [class] can contain multiple classes, figure_ or shape_ may not be the first.
          const locator = page.locator(
            `figure[class*='figure_'] div[class*='shape_'] svg:nth-of-type(${index + 1})`,
          );
    ...
    test('has correct content', async () => {
      ...
      await Promise.all(
        Object.entries(content).map(async ([tag, name]) => {
          expect(await page.textContent(`main section article ${tag}`)).toMatchSnapshot(
            `${name}.txt`,
          );
        }),
      );
